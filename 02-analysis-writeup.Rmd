...
p_cols <- c("Single blind" = "#7AD151FF", "Double blind" =  "#2A788EFF",
            "Not blinded" =  "#414487FF",
...
p_cols <- c("Not specified" =  "#FDE725FF", "Yes" = "#414487FF",
            "Conditional" =  "#2A788EFF",
            "Optional" = "#22A884FF", "No" = "#7AD151FF"
)
...
p_cols <- c("Unsure" = "#A6761D", "Yes" = "#414487FF",
            "No" = "#7AD151FF"
)
...
p1_cols <- c("No preprint policy" =  "#FDE725FF", "Other" = "#666666",
            "Any" = "#414487FF", "After peer review" = "#2A788EFF", 
            "First submission only (before peer review)" = "#22A884FF",
            "Unsure (preprints are allowed, but it's not clear which version)" = "#440154FF",
            "None" = "#7AD151FF"
)
...
p2_cols <- c("Not specified" =  "#FDE725FF", "Other" = "#666666", "Unsure" = "#A6761D",
            "Yes, in the reference list" = "#414487FF", 
            "Yes, but only in the text" = "#22A884FF", 
            "No" = "#7AD151FF"
)
...
mca_two_dimension_plot <- final_4 %>%
  ...
  coord_fixed() +
  theme(legend.position = "top") +
  scale_color_manual(values = c("Contributing Variables" = "#440154FF",
                                "Supplementary Variables" = "#22A884FF")) 
```
...
clarity_data <- refined %>%
  ...
  select(co_review, preprint_posting, preprint_citing, preprint_link,
...
p_cols <- c("Clear policy" = "#414487FF",
            "No clear policy" = "#7AD151FF"
)
