...
refined_with_areas %>%
  get_univ_data(area) %>% 
  select(area, n, prop, prop_numeric) %>% 
...
peer_type_data %>% 
  select(-order, peer_review_type = pr_type_clean) %>% 
...
pdata %>% 
  left_join(labels) %>% 
  select(variable = var, label, value = val_clean, n, prop) %>% 
  mutate(label = str_replace_all(label, "\\\n", " ")) %>% 
  write_csv("data/figures/Fig4_A.csv")


rev_identitiy %>% 
  select(-order, open_peer_review.identities_revealed_to_authors = opr_indenties_author_clean) %>% 
...
co_rev %>% 
  select(-order, can_coreviewers_contribute = coreview_email) %>% 
...
preprint_version %>% 
  select(-order, preprint_version = preprint_version_clean) %>% 
  write_csv("data/figures/Fig6_A.csv")

preprint_citation %>% 
    select(-order, preprint_citation = preprint_citation_clean) %>% 
