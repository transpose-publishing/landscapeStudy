...
refined_with_areas %>% 
  ...
  geom_jitter(data = filter(refined_with_areas, area_was_scraped),
              width = .2, colour = "black", show.legend = F, alpha = .7, 
              shape = 17) +
  labs(x = NULL) +
  scale_color_brewer(palette = "Dark2")  +
  ggrepel::geom_text_repel(
    data = tibble(
      area = "Health & Medical Sciences", `h5-index` = 115, 
      label = "Journals added\nby scraping"
    ), aes(label = label), nudge_y = -60, nudge_x = -.5, family = "Hind",
    size = 3, arrow = arrow(type = "closed", angle = 20, 
...
stemmed_words %>% 
  count(word, sort = T) %>% 
  mutate(proportion = {n/sum(n)} %>% scales::percent()) %>% 
  head(20) %>% 
...
```{r}
editor_propensity <- coreview_policies %>% 
  distinct() %>% 
  mutate(contains_editor = str_detect(str_to_lower(coreview_policy), "editor")) %>% 
  summarise(prop_editor = mean(contains_editor)) %>% 
  make_percent(2)
# 71%
```
...
refined_with_areas <- refined_with_areas %>% 
  ...
  mutate(preprint_version_clean = fct_relevel(
    preprint_version_clean, "Any", "First submission only (before peer review)") %>% 
