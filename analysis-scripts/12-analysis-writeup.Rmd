...
```{r pr-type-overall, fig.asp=.5, fig.cap="Type of peer review"}
...
```{r peer-type, fig.cap="Type of peer review by discipline", fig.asp=.6}
...
pr_no <- refined %>% 
  make_single_proportion(pr_database, "No")
```
...
`r pr_no` of journals deposit reviewer activity into databases which are not 
open. 
...
(`r pdata %>% filter(var == "opr_indenties_author") %>% pull(order) %>% unique() %>%  scales::percent()`), we examine it
...
```{r plot mca one dimensionally, fig.width=10, fig.height=8}
# get data from MCA
...
data_for_one_dimension %>% 
  ...
  labs(x = NULL, y = NULL, title = "Clarity of Policies")
```
...
`r final_4$inertia.e[1] %>% scales::percent()` of total variance. The second
dimension which explains 
`r final_4$inertia.e[2] %>% scales::percent()` of total variance is mainly driven by whether 
...
final_4 %>%
  ...
  labs(title = "The Landscape of Open Science Policies",
       caption = paste(
...
clarity_data %>% 
  left_join(labels) %>% 
  mutate(tabulation = glue::glue("{n} ({scales::percent(prop)})")) %>% 
  ungroup() %>% 
...
# The bigger picture
```{r}
transpose_db <- read_csv(here::here("data-transformed/transpose_db_data_verified.csv"))

display_table <- function(df, var) {
  df %>% 
    count({{var}}, sort = T) %>% 
...
transpose_db %>% 
  mutate(preprint_citation = str_trunc(preprint_citation, width = 100)) %>% 
  display_table(preprint_citation)


transpose_db %>% 
  display_table(preprint_version)

transpose_db %>% 
  display_table(opr_indenties_author)

transpose_db %>% 
  display_table(opr_identities_published)

transpose_db %>% 
  display_table(pr_type)
