...
mca_data <- refined_with_areas %>%
  mutate(
    ...
    identities_revealed = case_when(
      opr_indenties_author == "No" | opr_indenties_author == "Not specified" ~ "Reviewer identities not\nrevealed to authors",
      TRUE ~ "Reviewer identities\nrevealed to authors"
    ),
    pr_type_clean = fct_recode(pr_type_clean, 
                               `Other type of peer review` = "Other",
                               `Peer review ??` = "Unsure")
  )
```
...
```{r plot-mca, fig.width=12, fig.height=8, fig.cap="Joint analysis of open science policies"}
final_4 %>%
